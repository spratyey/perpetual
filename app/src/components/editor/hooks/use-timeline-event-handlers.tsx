...
interface UseTimelineEventHandlersProps {
  ...
  // Reference to the ticks/ruler area (ghost marker only shows when hovering here)
  ticksRef: React.RefObject<HTMLDivElement>;
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      if (isDragging) {
        ...
        // Only show ghost marker when hovering over the ticks/ruler area
        const ticksRect = ticksRef.current?.getBoundingClientRect();
        if (ticksRect && e.clientY >= ticksRect.top && e.clientY <= ticksRect.bottom) {
          ...
          // Mouse is over clips area, hide ghost marker
          setGhostMarkerPosition(null);
        }
      }
    },
    [isDragging, handleDrag, timelineRef, ticksRef, setGhostMarkerPosition]
  );
